...
class TestLegacyModelDictionaries(unittest.TestCase):
    def setUp(self) -> None:
        self.nested = client.NestedModel(camelCase="nested")
        self.model = client.LegacyModel(
            _continue="token",
            renamed="value",
            readOnlyValue="readonly",
            nested=self.nested,
            nestedList=[self.nested],
            nestedMap={"key": self.nested},
    ...
    def test_additional_properties_remain_transport_only(self) -> None:
        model = client.AdditionalPropertiesModel(declaredValue="declared")
    ...
    def test_custom_to_dict_implementations(self) -> None:
        ...
        class WrappedCustomLegacyModel(client.LegacyModel):
            @functools.wraps(client.LegacyModel.to_dict)
            def to_dict(
    ...
    def test_partial_constructed_model_emits_none(self) -> None:
        ...
        expected = {key: None for key in self.public_model_dict}
